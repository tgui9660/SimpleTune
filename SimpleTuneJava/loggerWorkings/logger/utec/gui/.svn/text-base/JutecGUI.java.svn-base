...
import logger.utec.gui.bottomControl.BottomUtecControl;
import logger.utec.gui.mapTabs.MapJPanel;
import logger.utec.gui.mapTabs.UtecDataManager;
import logger.utec.gui.realtimeData.RealTimeData;
import logger.utec.tts.UtecSpeaker;
...
public class JutecGUI extends JFrame implements ActionListener, KeyListener {
	...
	private JutecGUI(int setDefaultCloseOperation) {
		...
		logger.info("UTEC Gui is loading now.");
		
		// Actions to take when window is closing
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				logger.info("JUTEC Exiting");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.textInput) {
			logger.info("Text Input received:"+this.textInput.getText());
			String tempString = this.textInput.getText();
			tempString = tempString.substring(0, 1);
			
			logger.info("Reduce Input:"+tempString+":");
		...
		// Else this is either a port event or other
			...
			// Start Capture
				...
				logger.info("Load Map From File");
				...
				UtecInterface.closeConnection();
				logger.info("Exit action occuring");
	...
	*/
	...
	public void keyPressed(KeyEvent arg0) {
		...
		// logger.info("Key Pressed:"+arg0.getKeyCode()+"   :"+arg0.getKeyChar()+"   :"+arg0.getModifiers()+"     :"+(int)arg0.getKeyChar());
		
		if((int)arg0.getKeyChar() != 65535){
			int charCodeInt = (int)arg0.getKeyChar();
			logger.info("Key Code entered:"+charCodeInt);
	...
	public void keyTyped(KeyEvent arg0) {
		//logger.info("Key Typed:"+arg0.getKeyCode()+"   :"+arg0.getKeyChar()+"   :"+arg0.getModifiers());
		// TODO Auto-generated method stub
