...
public class Labels {
    ...
    public Labels( Axis axis, GraphData graphData ){
        zmax = graphData.get_Z_Max();
        zLabel = graphData.get_Z_Label();
        yLabel = graphData.get_Y_Label();
        xLabel = graphData.get_X_Label();
        title = graphData.getTitle();
        xmax = graphData.get_X_Max();
        ymax = graphData.get_Y_Max();
	...
	Text3D txt = new Text3D(f3d, String.valueOf( nf.format(ymax) ), 
	                     new Point3f( -.8f/.05f, 1f/.05f, 0f));
	yLabels[0].setGeometry(txt);
	    
	Text3D txt1 = new Text3D(f3d, String.valueOf( nf.format(ymax/2) ), 
	                     new Point3f( -.8f/.05f, .5f/.05f, 0f));
	yLabels[1].setGeometry(txt1);
	...
	for(int i=0; i<4; i++)trans.addChild(zLabels[i]);
